...
from i18n.catalog import ROOT, load_catalog, page_url


def alternate_links(registry, topic=None):
    links = {code: registry['site'] + page_url(registry, code, topic) for code in registry['locales']}
    links['x-default'] = links[registry['defaultLocale']]
...
def navigation(registry, code, topic=None, mark_current=True):
    ...
    for language, config in registry['locales'].items():
        ...
        if not topic and language == registry['defaultLocale']:
            path += '?lang=' + language
...
def render(registry, catalog, code, template, topic=None):
    ...
    if topic:
        ...
        related = ''.join(f'<li><a href="{page_url(registry, code, key)}">{escape(pages[key]["title"])}</a></li>' for key in registry['topics'] if key != topic)
...
<aside class="related"><h2>{t('navigation.related')}</h2><ul>{related}</ul><a href="{registry['topics'][topic]['relatedEnglish']}?lang=en">{t('navigation.englishResource')}</a></aside>'''
        ...
        cards = ''.join(f'<section class="section"><h2><a href="{page_url(registry, code, key)}">{escape(pages[key]["title"])}</a></h2><p>{escape(pages[key]["intro"])}</p><a href="{page_url(registry, code, key)}">{t("navigation.openGuide")}</a></section>' for key in registry['topics'])
...
def outputs(root=ROOT):
    ...
    for code in registry['locales']:
        if code != registry['defaultLocale']:
            result[root / f'{code}.html'] = render(registry, catalogs[code], code, template)
    ...
    for topic, config in registry['topics'].items():
        ...
        result[path] = replace_block(path.read_text(), 'i18n-navigation', '')
    ...
    # /en itself has no separate hub; it redirects to the existing English home.
    nondefault = '|'.join(code for code in registry['locales'] if code != registry['defaultLocale'])
    rules = f'{start}\nRewriteRule ^en/?$ / [R=301,L]\nRewriteRule ^({nondefault})/$ /$1 [R=301,L]\nRewriteRule ^({nondefault})$ $1.html [L]\n{end}'
...
def main():
    ...
    guides = languages * len(registry['topics'])
