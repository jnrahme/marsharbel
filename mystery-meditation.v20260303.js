...
sessionControls.className = 'meditation-top-controls';
sessionControls.innerHTML = `
  <a href="${basePath}rosary-intro.html" class="btn subtle">Rosary Intro</a>
  <button id="start-set-prayer" type="button" class="btn subtle">Pray This Set (5)</button>
  <button id="start-today-prayer" type="button" class="btn subtle">Pray Today's Rosary (Audio Guided)</button>
`;
if (heroSection && topMysteryControls) {
  heroSection.insertBefore(sessionControls, topMysteryControls.nextSibling);
}
const startSetPrayerButton = document.getElementById('start-set-prayer');
const startTodayPrayerButton = document.getElementById('start-today-prayer');
...
const speakPrayer = (prayerKey, onComplete = null) => {
...
const speakPrayerSequence = (sequence, onComplete = null) => {
...
const playClipIfAvailable = async (currentStageIndex, narrationText) => {
  if (!clipAvailable) {
    return false;
  }
  ...
  clipAudio.onended = () => handleNarrationComplete(currentStageIndex);
  clipAudio.onpause = () => {
    if (!clipAudio.ended) {
      syncGlobalAudioContext({ mode: 'prerendered', playing: true, paused: true, pausedAt: Date.now() });
    }
...
const playVoice = () => {
  ...
  playClipIfAvailable(currentStageIndex, narrationText).then(played => {
    if (played) {
      return;
    }
    const stagePrayerSequence = prayerSequenceForStage(currentStageIndex);
...
if (startSetPrayerButton) {
  startSetPrayerButton.addEventListener('click', () => startSetSession(currentSet));
}

if (startTodayPrayerButton) {
  startTodayPrayerButton.addEventListener('click', () => startSetSession(detectTodaySet()));
}
...
if (autoPrayerToggle) {
  autoPrayerToggle.addEventListener('change', () => {
    ...
    autoTimerToggle.disabled = autoPrayerVoiceEnabled;
    autoTimerSeconds.disabled = autoPrayerVoiceEnabled;
...
autoTimerToggle.addEventListener('click', () => {
  if (autoPrayerVoiceEnabled) {
    soundStatus.textContent = 'Auto timer is disabled while Auto Prayer Sequence is enabled.';
    return;
  }
...
updateVoiceButtonLabel();
studioPlayButton.textContent = 'Play Studio Track';
if (autoTimerToggle && autoTimerSeconds) {
  autoTimerToggle.disabled = autoPrayerVoiceEnabled;
  autoTimerSeconds.disabled = autoPrayerVoiceEnabled;
}
ensureClipPlayers();
