(function () {
  ...
  const intakeOpen = cfg.submissionsEnabled === true && turnstileReady && endpointReady;
  ...
  form?.addEventListener('submit', async (event) => {
    ...
    try {
      const response = await fetch(cfg.submissionEndpoint,{method:'POST',headers:{'Content-Type':'application/json','Accept':'application/json'},credentials:'omit',cache:'no-store',body:JSON.stringify(payload)});
